...
//    only - a typed BerufsbildungDatenExporteContext there would make every ctx
...
final class BerufsbildungDatenExporteOmni
{
    ...
    public static function ctx_from_map($ctxmap, $client, $utility)
    {
        ...
        $ctx = new BerufsbildungDatenExporteContext($clean, null);
        ...
        if (isset($clean['spec']) && is_array($clean['spec'])) {
            $ctx->spec = new BerufsbildungDatenExporteSpec($clean['spec']);
        }

        if (isset($clean['result']) && is_array($clean['result'])) {
            $ctx->result = new BerufsbildungDatenExporteResult($clean['result']);
            if (isset($clean['result']['err']) && is_array($clean['result']['err'])) {
                $msg = $clean['result']['err']['message'] ?? '';
                $ctx->result->err = new BerufsbildungDatenExporteError('', is_string($msg) ? $msg : '');
            }
        }

        if (isset($clean['response']) && is_array($clean['response'])) {
            $ctx->response = new BerufsbildungDatenExporteResponse($clean['response']);
    ...
    public static function err_from_map($m): ?BerufsbildungDatenExporteError
    {
        if (!is_array($m)) {
            return null;
        }
        ...
        $code = $m['code'] ?? '';
        return new BerufsbildungDatenExporteError(is_string($code) ? $code : '', $msg);
    }
