...
// BerufsbildungDatenExporte SDK retry feature
...
class BerufsbildungDatenExporteRetryFeature extends BerufsbildungDatenExporteBaseFeature
{
    ...
    public function init(BerufsbildungDatenExporteContext $ctx, array $options): void
    {
        ...
        $utility->fetcher = function (BerufsbildungDatenExporteContext $ctx2, string $url, array $fetchdef) use ($inner): array {
            return $this->_with_retry($ctx2, $url, $fetchdef, $inner);
        };
    }

    private function _with_retry(BerufsbildungDatenExporteContext $ctx, string $url, array $fetchdef, callable $inner): array
    {
        $opts = $this->options ?? [];
    ...
    private function _track(BerufsbildungDatenExporteContext $_ctx, int $attempt, mixed $res, mixed $err, float $wait): void
    {
        $client = $this->client;
