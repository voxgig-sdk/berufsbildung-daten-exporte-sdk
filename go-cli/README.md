# berufsbildung-daten-exporte-cli

boru-driven command-line client **and** interactive REPL for the BerufsbildungDatenExporte
...
```sh
# 1. Build a native binary (-> dist/<os>-<arch>/berufsbildung-daten-exporte-cli)
make build

# 2. See usage (words, entities, env vars)
./berufsbildung-daten-exporte-cli --help

# 3. Provide credentials once, via the environment
export BERUFSBILDUNG_DATEN_EXPORTE_APIKEY=sk_live_xxx

# 4. Each command line is ONE boru expression, run against the API:
./berufsbildung-daten-exporte-cli list berufsbildung
./berufsbildung-daten-exporte-cli load 1 berufsbildung            # {id:1} shorthand
./berufsbildung-daten-exporte-cli load '{id:1}' berufsbildung       # explicit match map

# 5. Override the API base URL for a single call
BERUFSBILDUNG_DATEN_EXPORTE_BASE=https://api.example.com ./berufsbildung-daten-exporte-cli list berufsbildung

# 6. No arguments -> interactive REPL
./berufsbildung-daten-exporte-cli
berufsbildung-daten-exporte> list berufsbildung
berufsbildung-daten-exporte> /quit
```
...
1. **Build the binary.** From this `go-cli/` directory:
   ...
   ```sh
   make build          # -> dist/<os>-<arch>/berufsbildung-daten-exporte-cli
   ```
...
3. **Run a query.** Evaluate an boru expression against the API (or run with no
   ...
   ```sh
   ./dist/*/berufsbildung-daten-exporte-cli list berufsbildung
   ```
...
```sh
./berufsbildung-daten-exporte-cli load 1 berufsbildung          # scalar shorthand for {id:1}
./berufsbildung-daten-exporte-cli load '{id:1}' berufsbildung     # explicit match map
```
...
Run with no arguments to open a REPL (prompt `berufsbildung-daten-exporte>`). Each line is
evaluated as its own boru expression:

```text
$ ./berufsbildung-daten-exporte-cli
berufsbildung-daten-exporte> list berufsbildung
berufsbildung-daten-exporte> /help
berufsbildung-daten-exporte> /quit
```
...
- `<entity>` is a bareword, auto-quoted as an boru atom (e.g. `berufsbildung`).
...
| Target | Result |
|--------|--------|
| `make build` | Native binary at `dist/<os>-<arch>/berufsbildung-daten-exporte-cli`. |
...
(`./berufsbildung-daten-exporte-cli <expr>`) and interactively (the REPL), and expressions compose the
