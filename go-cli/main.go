// Command BerufsbildungDatenExporte-cli is a CLI + REPL for the BerufsbildungDatenExporte SDK,
...
const prompt = "berufsbildung-daten-exporte"

// entitiesHelp is the space-separated entity list shown by /help.
const entitiesHelp = "berufsbildung"
...
func usage(out io.Writer) {
	...
	fmt.Fprintln(out, "Env:      BERUFSBILDUNG_DATEN_EXPORTE_APIKEY (api key), BERUFSBILDUNG_DATEN_EXPORTE_BASE (base url override)")
...
func run(args []string, in io.Reader, out, errOut io.Writer) int {
	...
	// Configure from the environment: BERUFSBILDUNG_DATEN_EXPORTE_APIKEY carries the API key and
	// BERUFSBILDUNG_DATEN_EXPORTE_BASE optionally overrides the API base URL (e.g. production).
	// Both injectable by a secrets vault. Unset -> nil config defaults.
	var opts map[string]any
	if apikey := os.Getenv("BERUFSBILDUNG_DATEN_EXPORTE_APIKEY"); apikey != "" {
		opts = map[string]any{"apikey": apikey}
	}
	if base := os.Getenv("BERUFSBILDUNG_DATEN_EXPORTE_BASE"); base != "" {
		if opts == nil {
			opts = map[string]any{}
		}
		opts["base"] = base
	}
	client := sdk.NewBerufsbildungDatenExporteSDK(opts)
