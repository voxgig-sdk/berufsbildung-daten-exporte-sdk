...
func envOverride(m map[string]any) map[string]any {
	if os.Getenv("BERUFSBILDUNGDATENEXPORTE_TEST_LIVE") == "TRUE" ||
		os.Getenv("BERUFSBILDUNGDATENEXPORTE_TEST_OVERRIDE") == "TRUE" {
		for key := range m {
	...
	if explain := os.Getenv("BERUFSBILDUNGDATENEXPORTE_TEST_EXPLAIN"); explain != "" {
