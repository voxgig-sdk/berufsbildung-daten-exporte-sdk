...
func TestBerufsbildungEntity(t *testing.T) {
	...
	t.Run("stream", func(t *testing.T) {
		...
		var seen []any
		for item := range base.Berufsbildung(nil).Stream("list", nil, nil) {
		...
		if hasStreaming {
			...
			var got []any
			for item := range streamSdk.Berufsbildung(nil).Stream("list", nil, nil) {
	...
	t.Run("basic", func(t *testing.T) {
		...
		if setup.syntheticOnly {
			t.Skip("live entity test uses synthetic IDs from fixture — set BERUFSBILDUNG_DATEN_EXPORTE_TEST_BERUFSBILDUNG_ENTID JSON to run live")
			return
		}
		client := setup.client
...
func berufsbildungBasicSetup(extra map[string]any) *entityTestSetup {
	...
	entidEnvRaw := os.Getenv("BERUFSBILDUNG_DATEN_EXPORTE_TEST_BERUFSBILDUNG_ENTID")
	...
	idmapResolved := core.ToMapAny(env["BERUFSBILDUNG_DATEN_EXPORTE_TEST_BERUFSBILDUNG_ENTID"])
