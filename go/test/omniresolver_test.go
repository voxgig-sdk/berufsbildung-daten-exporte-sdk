...
import (
	...
	omni "github.com/voxgig-sdk/berufsbildung-daten-exporte-sdk/go/test/omni"
...
func omniProvider(client any) *omni.Provider {
	provider := &omni.Provider{
		...
		Errify: func(err any) any {
			if sdkerr, is := err.(*sdk.BerufsbildungDatenExporteError); is {
				out := map[string]any{
					"name":    "BerufsbildungDatenExporteError",
...
func omniCtx(arg any, client *sdk.BerufsbildungDatenExporteSDK, utility *sdk.Utility) *sdk.Context {
	...
	if provider, is := ctxmap["client"].(*omni.Provider); is {
		if live, is := defproviderclient(provider).(*sdk.BerufsbildungDatenExporteSDK); is {
			client = live
