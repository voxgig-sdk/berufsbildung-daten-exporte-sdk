...
[MCP](https://modelcontextprotocol.io) server exposing the BerufsbildungDatenExporte SDK as
two agent tools — `berufsbildung-daten-exporte_list` and `berufsbildung-daten-exporte_load` — built on the
...
```sh
# 1. Build a native binary (-> dist/<os>-<arch>/berufsbildung-daten-exporte-mcp)
make build

# 2. Provide credentials via the environment
export BERUFSBILDUNG_DATEN_EXPORTE_APIKEY=sk_live_xxx
...
```jsonc
// berufsbildung-daten-exporte_list: first page of records
{ "entity": "berufsbildung" }
{ "entity": "berufsbildung", "query": { } }

// berufsbildung-daten-exporte_load: one record by id
{ "entity": "berufsbildung", "query": { "id": 1 } }
```
...
1. **Build** the server from this `go-mcp/` directory:
   ...
   ```sh
   make build          # -> dist/<os>-<arch>/berufsbildung-daten-exporte-mcp
   ```
...
3. **Install it into Claude Code** (stdio transport):
   ...
   ```sh
   claude mcp add --scope user berufsbildung-daten-exporte \
     -- "$PWD"/dist/*/berufsbildung-daten-exporte-mcp -transport stdio
   ```

4. **Restart Claude Code.** The `berufsbildung-daten-exporte_list` and `berufsbildung-daten-exporte_load` tools now appear
   in new sessions. Ask the agent to *"list berufsbildung using berufsbildung-daten-exporte"*
   and it calls `berufsbildung-daten-exporte_list` with `{"entity":"berufsbildung"}`.
...
```sh
export BERUFSBILDUNG_DATEN_EXPORTE_APIKEY=sk_live_xxx            # API key
export BERUFSBILDUNG_DATEN_EXPORTE_BASE=https://api.example.com  # optional: override the API base URL
```
...
### Call the `berufsbildung-daten-exporte_list` tool
...
### Call the `berufsbildung-daten-exporte_load` tool
...
| Tool | Args | Returns |
|------|------|---------|
| `berufsbildung-daten-exporte_list` | `entity` (required), `query` (optional map) | First page of records as JSON |
| `berufsbildung-daten-exporte_load` | `entity` (required), `query` = `{id:N}` | Single record as JSON |
...
| Variable | Purpose |
|----------|---------|
| `BERUFSBILDUNG_DATEN_EXPORTE_APIKEY` | API key sent with every request. |
| `BERUFSBILDUNG_DATEN_EXPORTE_BASE` | Optional override of the API base URL. |
