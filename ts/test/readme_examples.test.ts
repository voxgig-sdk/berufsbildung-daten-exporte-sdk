...
//     (\`new BerufsbildungDatenExporteSDK(...)\` / \`BerufsbildungDatenExporteSDK.test(...)\` become
//     \`BerufsbildungDatenExporteSDK.test({ entity: ... })\`), transpiled to JS, and run
...
const SDK_NAME = 'BerufsbildungDatenExporteSDK'
...
const TEST_SEED = {"entity":{"berufsbildung":{"test01":{"id":"test01"}}}}
...
// rewrite real client construction (\`new BerufsbildungDatenExporteSDK(...)\` and any
// \`BerufsbildungDatenExporteSDK.test(...)\`) to a seeded test-mode client, and give
...
function rewriteForRun(code: string): string {
  ...
  out = out.replace(/new\s+BerufsbildungDatenExporteSDK\s*\([^)]*\)/g, () => SEEDED_CTOR)
  out = out.replace(/BerufsbildungDatenExporteSDK\.test\s*\([^)]*\)/g, () => SEEDED_CTOR)
