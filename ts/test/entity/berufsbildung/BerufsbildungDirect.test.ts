...
describe('BerufsbildungDirect', async () => {
  ...
  test('direct-load-berufsbildung', async (t: any) => {
    if (liveScenariosActive()) { t.skip('Covered by live operation scenarios'); return }
    const setup = directSetup({ id: 'direct01' })
    if (maybeSkipControl(t, 'direct', 'direct-load-berufsbildung', setup.live)) return
    if (skipIfMissingIds(t, setup, ["format01"])) return
  ...
  test('direct-list-berufsbildung', async (t: any) => {
    if (liveScenariosActive()) { t.skip('Covered by live operation scenarios'); return }
    const setup = directSetup([{ id: 'direct01' }, { id: 'direct02' }])
    if (maybeSkipControl(t, 'direct', 'direct-list-berufsbildung', setup.live)) return
    const { client, calls } = setup
