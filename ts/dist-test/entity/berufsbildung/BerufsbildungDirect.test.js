...
(0, node_test_1.describe)('BerufsbildungDirect', async () => {
    ...
    (0, node_test_1.test)('direct-exists', async () => {
        const sdk = new __1.BerufsbildungDatenExporteSDK({
    ...
    (0, node_test_1.test)('direct-load-berufsbildung', async (t) => {
        if (liveScenariosActive()) {
            t.skip('Covered by live operation scenarios');
            return;
        }
        const setup = directSetup({ id: 'direct01' });
        if ((0, utility_1.maybeSkipControl)(t, 'direct', 'direct-load-berufsbildung', setup.live))
            return;
        if ((0, utility_1.skipIfMissingIds)(t, setup, ["format01"]))
            return;
        ...
        if (setup.live) {
            ...
            const candidateId = listArr[0]?.format ?? listArr[0]?.id;
            if (null == candidateId) {
                throw new Error('Live load blocked: discovery returned no usable identity');
            }
            params.format = candidateId;
    ...
    (0, node_test_1.test)('direct-list-berufsbildung', async (t) => {
        if (liveScenariosActive()) {
            t.skip('Covered by live operation scenarios');
            return;
        }
        const setup = directSetup([{ id: 'direct01' }, { id: 'direct02' }]);
        if ((0, utility_1.maybeSkipControl)(t, 'direct', 'direct-list-berufsbildung', setup.live))
            return;
        const { client, calls } = setup;
        const params = {};
        const query = {};
...
function liveScenariosActive() { return false && process.env.BERUFSBILDUNG_DATEN_EXPORTE_TEST_LIVE === 'TRUE'; }
function directSetup(mockres) {
    ...
    if (live) {
        ...
        const client = new __1.BerufsbildungDatenExporteSDK(Object.assign({}, (0, utility_1.liveClientOptions)(), { system: { fetch: transport.fetch },
        }));
        let idmap = env['BERUFSBILDUNG_DATEN_EXPORTE_TEST_BERUFSBILDUNG_ENTID'];
    ...
    const client = new __1.BerufsbildungDatenExporteSDK({
        base: 'http://localhost:8080',
...
//# sourceMappingURL=BerufsbildungDirect.test.js.map