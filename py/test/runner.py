# BerufsbildungDatenExporte SDK test runner
...
class BerufsbildungDatenExporteTestRunner:
    _env = {}
    ...
    def load_env_local():
        ...
        for line in content.splitlines():
            ...
            BerufsbildungDatenExporteTestRunner._env[key] = val

    @staticmethod
    def getenv(key):
        val = BerufsbildungDatenExporteTestRunner._env.get(key)
        if val is not None:
            return val
    ...
    @staticmethod
    def env_override(m):
        live = BerufsbildungDatenExporteTestRunner.getenv("BERUFSBILDUNG_DATEN_EXPORTE_TEST_LIVE")
        override = BerufsbildungDatenExporteTestRunner.getenv("BERUFSBILDUNG_DATEN_EXPORTE_TEST_OVERRIDE")

        if live == "TRUE" or override == "TRUE":
            for key in list(m.keys()):
                envval = BerufsbildungDatenExporteTestRunner.getenv(key)
        ...
        explain = BerufsbildungDatenExporteTestRunner.getenv("BERUFSBILDUNG_DATEN_EXPORTE_TEST_EXPLAIN")
    ...
    def load_test_control():
        ...
        """
        if BerufsbildungDatenExporteTestRunner._test_control is not None:
            return BerufsbildungDatenExporteTestRunner._test_control
        ctrl_path = os.path.join(os.path.dirname(__file__), "sdk-test-control.json")
        try:
            with open(ctrl_path, "r") as f:
                BerufsbildungDatenExporteTestRunner._test_control = json.load(f)
        except (FileNotFoundError, IOError, ValueError):
            BerufsbildungDatenExporteTestRunner._test_control = {
                "version": 1,
                "test": {"skip": {
    ...
    def is_control_skipped(kind, name, mode):
        ...
        ctrl = BerufsbildungDatenExporteTestRunner.load_test_control()
...
def load_env_local():
    BerufsbildungDatenExporteTestRunner.load_env_local()


def env_override(m):
    return BerufsbildungDatenExporteTestRunner.env_override(m)


def entity_data(v):
    return BerufsbildungDatenExporteTestRunner.entity_data(v)


def entity_list_to_data(lst):
    return BerufsbildungDatenExporteTestRunner.entity_list_to_data(lst)


def is_control_skipped(kind, name, mode):
    return BerufsbildungDatenExporteTestRunner.is_control_skipped(kind, name, mode)


def load_test_control():
    return BerufsbildungDatenExporteTestRunner.load_test_control()


def live_client_options():
    return BerufsbildungDatenExporteTestRunner.live_client_options()


def live_delay_ms():
    return BerufsbildungDatenExporteTestRunner.live_delay_ms()
