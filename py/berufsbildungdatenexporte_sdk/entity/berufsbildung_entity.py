...
from berufsbildungdatenexporte_sdk.berufsbildungdatenexporte_types import (
    Berufsbildung,
...
class BerufsbildungEntity:
    ...
    def make(self):
        ...
        for k, v in self._entopts.items():
            opts[k] = v
        return BerufsbildungEntity(self._client, opts)
    ...
    def data_get(self) -> Berufsbildung:
    ...
    def match_get(self) -> Berufsbildung:
    ...
    def load(self, reqmatch=None, ctrl=None) -> Berufsbildung:
        utility = self._utility
        # reqmatch is optional: an entity with no id-like key loads with no
        # match. Treat None as an empty match so client.Berufsbildung().load()
        # works with no args.
        if reqmatch is None:
            reqmatch = {}
    ...
    def list(self, reqmatch=None, ctrl=None) -> list[Berufsbildung]:
        utility = self._utility
        # reqmatch is optional: an omitted match lists all records. Treat None
        # as an empty match so client.Berufsbildung().list() works with no args.
        if reqmatch is None:
            reqmatch = {}
        ctx = utility.make_context({
            "opname": "list",
