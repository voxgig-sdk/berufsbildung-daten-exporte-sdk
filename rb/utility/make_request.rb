...
module BerufsbildungDatenExporteUtilities
  MakeRequest = ->(ctx) {
    ...
    spec = ctx.spec
    utility = ctx.utility
    response = BerufsbildungDatenExporteResponse.new({})
    ...
    if fetch_err
      ...
      response = BerufsbildungDatenExporteResponse.new({ "err" => ctx.make_error("request_no_response", "response: undefined") })
    elsif fetched.is_a?(Hash)
      response = BerufsbildungDatenExporteResponse.new(fetched)
    else
